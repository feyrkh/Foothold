...
func set_parent_item(command_tree:CommandTree, item_id):
	...
	add_item('Rename', CMD_RENAME_FOLDER)
...
func _on_Popup_id_pressed(id: int) -> void:
	...
	match cmd_id:
		CMD_NEW_FOLDER:
			var popup:PopupPanel = preload("res://ui/popup/CreateFolderPopup.tscn").instance()
			popup.set_parent_item(parent_tree, parent_item_id)
			add_child(popup)
			popup.popup_centered()
		CMD_RENAME_FOLDER:
			var popup:PopupPanel = preload("res://ui/popup/RenameFolderPopup.tscn").instance()
			popup.set_parent_item(parent_tree, parent_item_id)
			add_child(popup)
			popup.popup_centered()
		CMD_DELETE_FOLDER:
			var popup:PopupPanel = preload("res://ui/popup/DeleteFolderPopup.tscn").instance()
			popup.set_parent_item(parent_tree, parent_item_id)
			add_child(popup)
			popup.popup_centered()
