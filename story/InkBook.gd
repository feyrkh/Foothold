...
var ink_book_save_data:InkBookSaveData
...
func rerender_page(page, page_idx):
	...
	for sentence in sentences:
		if sentence.begins_with(IMAGE):
			...
			page.add_child(img)
		elif sentence.begins_with(SIGNAL):
			trigger_signal(sentence.substr(SIGNAL.length()))
		else:
...
func trigger_signal(signal_text):
	var args = signal_text.split(':')
	var signal_name = args.pop_front()
	if args.length == 0:
		Events.emit_signal(signal_name)
	else:
		Events.emit_signal(signal_name, args)

func start_story(resource_name, ink_book_save_data:InkBookSaveData, notify_obj=null, notify_method=null):
	var file = load(resource_name)
	self.ink_book_save_data = ink_book_save_data
...
func init_story_globals():
	$InkPlayer.bind_external_function('signal', self, 'story_signal')
	$InkPlayer.bind_external_function('signal1', self, 'story_signal1')
	var vars = ink_book_save_data.story_globals
	for var_name in vars:
		$InkPlayer.set_variable(var_name, vars[var_name])

func story_signal(signal_name):
	Events.emit_signal(signal_name)

func story_signal1(signal_name, arg1):
	Events.emit_signal(signal_name, arg1)

func save_story_globals():
	if !ink_book_save_data:
		return
	var global_var_names = $InkPlayer._story.state.variables_state.enumerate()
	for var_name in global_var_names:
		var global_var_value = $InkPlayer._story.state.variables_state.get(var_name)
		ink_book_save_data.story_globals[var_name] = global_var_value
